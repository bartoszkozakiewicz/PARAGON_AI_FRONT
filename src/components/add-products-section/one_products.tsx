    ...
    <div className="flex flex-col gap-2">
      <div
        className={`flex flex-1 flex-row ${
          activeButton === 'Spozywcze' ? 'justify-center' : 'justify-start'
        } items-center gap-3`}
      >
        ...
        {activeButton === 'Spozywcze' && (
          <FormControl sx={{ minWidth: 120, flexBasis: '23%' }}>
            ...
            <Select
              ...
              onChange={(event) => {
                setProducts((prevProd: any) =>
                  prevProd.map((prod: any, index: number) => {
                    if (index !== num) {
                      return prod;
                    }
                    return {
              ...
              {list_categories &&
                list_categories.map((item: any) => (
                  <MenuItem
                    ...
                    value={`${item.category}`}
                  >
                    <p className="whitespace-normal overflow-hidden truncate min-w-full">
                      {item.category}
                    </p>
                  </MenuItem>
                ))}
        ...
        <DeleteRoundedIcon
          sx={{
            color: 'grey',
            cursor: 'pointer',
            marginLeft: activeButton === 'Spozywcze' ? '0px' : 'auto',
          }}
          onClick={() => handleDeleteProduct(num)}
        />
      </div>
