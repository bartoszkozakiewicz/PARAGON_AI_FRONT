...
const page = (props:any) => {
    ...
    useEffect(()=>{
        ...
        setShop({name:searchParams.get("shopName") as string,date:formattedDate as string})  

        const getParagon = async() =>{
            const data = await axiosInstance.get(`${path}/product/paragon?shopId=${searchParams.get("shopId")}`).then((res)=>setActualData(res.data)).catch(e=>console.error(e))
        }
        getParagon()
    },[])
    ...
    <div className='flex mx-auto items-center justify-center w-[80%] mt-10'>
        <div className="flex flex-col gap-3 p-6 shadow-custom rounded-lg w-[100%] min-h-[500px] max-h-[500px] items-center">
            <div className="flex flex-row items-left w-[100%] gap-4 justify-start">
                <TextField id="outlined-basic" label="Nazwa Sklepu" value={shop.name ? shop.name : ""} variant="outlined" onChange={((e)=>setShop((prevData:Shop)=>({...prevData,name:e.target.value})))}/>
